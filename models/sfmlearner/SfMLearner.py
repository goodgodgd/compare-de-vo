...
from models.sfmlearner.data_loader import DataLoader
from models.sfmlearner.nets import *
from models.sfmlearner.utils import *
from abstracts import ModelBase


class SfMLearner(ModelBase):
    ...
    def build_depth_test_graph(self, tgt_image):
        tgt_image = self.preprocess_image(tgt_image)
        with tf.name_scope("depth_prediction"):
            pred_disp, depth_net_endpoints = disp_net(
                tgt_image, is_training=False)
            pred_depth = [1./disp for disp in pred_disp]
        pred_depth = pred_depth[0]
        self.pred_depth = pred_depth
        self.depth_epts = depth_net_endpoints

    def build_pose_test_graph(self, tgt_image, src_image_stack):
        tgt_image = self.preprocess_image(tgt_image)
        src_image_stack = self.preprocess_image(src_image_stack)
    ...
    def build_model(self, tgt_image, src_image_stack, intrinsics):
        ...
        if opt.add_dispnet:
            self.build_depth_test_graph(tgt_image)

        if opt.add_posenet:
            self.build_pose_test_graph(tgt_image, src_image_stack)
